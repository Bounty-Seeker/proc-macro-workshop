...
use syn::{
    punctuated::Punctuated, spanned::Spanned, token::Colon2, Arm, Error, Ident, Pat, PatIdent,
    PatPath, PatTupleStruct, Path, PathSegment, Result,
};
    ...
    fn my_eq(&self, other: &Self) -> bool {
        ...
        matches!(
            ...
            (ArmTypes::Empty, ArmTypes::Empty)
                | (ArmTypes::Path, ArmTypes::Path)
                | (ArmTypes::Path, ArmTypes::Wild)
                | (ArmTypes::Wild, ArmTypes::Path)
                | (ArmTypes::TupleStruct, ArmTypes::TupleStruct)
                | (ArmTypes::TupleStruct, ArmTypes::Wild)
                | (ArmTypes::Wild, ArmTypes::TupleStruct)
                | (ArmTypes::Ident, ArmTypes::Ident)
                | (ArmTypes::Ident, ArmTypes::Wild)
                | (ArmTypes::Wild, ArmTypes::Ident)
...
impl<'a> ValidatedArms<'a> {
    ...
    pub fn validate_arms(arms: &'a [Arm]) -> Result<ValidatedArms<'a>> {
        ...
        if arms.is_empty() {
            let empty_match = ValidatedArms {
                arm_type: ArmTypes::Empty,
                arms,
            };
...
fn check_pat_sorted(arms: &[Arm]) -> Result<()> {
    let mut patterns_iter = arms.iter().map(|arm| &arm.pat);
...
fn get_pattern_ident(pat: &Pat) -> String {
    match pat {
        ...
        Pat::TupleStruct(tuple_struct) => format_punct(&tuple_struct.path.segments),
        Pat::Path(path) => format_punct(&path.path.segments),
