...
use syn::{
    ...
    token::{Colon2, Comma},
    Attribute, Data, DeriveInput, Error, Fields, GenericArgument, Ident, LitStr, Meta, MetaList,
    MetaNameValue, NestedMeta, Path, PathArguments, PathSegment, ReturnType, TraitBound, Type,
...
pub fn derive(input: TokenStream) -> TokenStream {
    ...
    let field_attrs = fields.into_iter().map(|field| field.attrs).map(|attr_vec| {
        let mut attr_iter = attr_vec
            .into_iter()
...
fn only_param_pthan_field<'a>(
    ...
    for field in fields {
        ...
        if let Type::Path(type_path) = &field_type {
            ...
            // Not PhantomData<T> case so if contains T then return false
            return false;
        }
...
fn find_assoc_types(param_ident: &Ident, field_type: &Type) -> Vec<Type> {
    ...
    match field_type {
        ...
        Type::ImplTrait(_type_impl_trait) => {
            panic!("Can't handle impl traits currently")
        }
        ...
        Type::Path(type_path) => {
            ...
            while let Some(segment) = path_segments_iter.next() {
                ...
                if is_ident {
                    let mut associ_path_segs: Punctuated<PathSegment, Colon2> =
                        Punctuated::<PathSegment, Colon2>::new();
        ...
        Type::TraitObject(_type_trait_object) => {
            panic!("Can't handle dynamic traits currently")
        }
        ...
        Type::Verbatim(_token_stream) => {
            panic!("Can't find from token_stream")
        }
        _ => panic!(),
    }
}
...
fn try_attribute(attr: &Attribute) -> Option<WherePredicate> {
    ...
    } = path
    {
        ...
        for nest in nested {
            if let NestedMeta::Meta(Meta::NameValue(name_value)) = nest {
                let MetaNameValue {
                    path,
                    eq_token: _,
                    lit,
                ...
                if let syn::Lit::Str(lit) = lit {
                    ...
                    let parse_attempt: Result<WherePredicate, Error> =
...
fn contains_param_type(param_ident: &Ident, to_check_against: &Type) -> bool {
    ...
    match to_check_against {
        ...
        Type::BareFn(type_bare_fn) => {
            let function_args = &type_bare_fn.inputs;
            for function_arg in function_args {
                if contains_param_type(param_ident, &function_arg.ty) {
                    return true;
                }
            }
            ...
            for segments in &type_path.path.segments {
                ...
                match &segments.arguments {
                        ...
                        for arg in args {
                            match arg {
                                ...
                                GenericArgument::Type(typ) => {
                                    if contains_param_type(param_ident, &typ) {
                                        return true;
                                    }
                                }
                                GenericArgument::Binding(binding) => {
                                    if contains_param_type(param_ident, &binding.ty) {
                                        return true;
                                    }
                                }
                                GenericArgument::Constraint(_constraint) => continue,
                                GenericArgument::Const(_expr) => {
                                    panic!("Can't tell as expr")
                                }
                            }
                        }
                    }
                    PathArguments::Parenthesized(args) => {
                        ...
                        for function_arg in function_args {
                            if contains_param_type(param_ident, &function_arg) {
                                return true;
                            }
                        }
        ...
        Type::Tuple(type_tuple) => {
            for tuple_term_type in &type_tuple.elems {
                if contains_param_type(param_ident, &tuple_term_type) {
                    return true;
                }
            }
            false
        ...
        Type::Verbatim(_token_stream) => {
            panic!("Can't tell as TokenStream")
        }
        _ => panic!(),
    }
}
